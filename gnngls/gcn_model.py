...
from gnngls.gcn_layers import ResidualGatedGCNLayer, MLP
from utils.model_utils import *
import pdb
import sys
sys.path.append('/home/suijingyan/sjy_data/suijingyan/realTSP/gnngls')
...
class ResidualGatedGCNModel(nn.Module):
    ...
    def __init__(self, config, dtypeFloat, dtypeLong):
        ...
        self.edges_embedding = nn.Embedding(self.voc_edges_in, self.hidden_dim//2) #self.voc_edges_in=3,是因为x_edges是3，及祠表里只有0，1，3这三个值
        # Define GCN Layers
        gcn_layers = []
        ...
        self.mlp_edges = MLP(self.hidden_dim, 1, self.mlp_layers)
        # self.mlp_nodes = MLP(self.hidden_dim, self.voc_nodes_out, self.mlp_layers)
        ### sjy 
        # self.mlp_edges = MLP(self.hidden_dim, 1, self.mlp_layers)
    
    #sjy
    def forward(self, x_nodes_coord, x_edges_values, x_edges):
        """
        Args:
